...
MAX_QUERY_CHARS = 400        # Máximo de caracteres por pregunta (evita consumo masivo de contexto)
MIN_QUERY_CHARS = 3          # Mínimo de caracteres válidos
MAX_WORD_COUNT = 80          # Máximo de palabras por mensaje
MAX_SINGLE_WORD_LENGTH = 45  # Máximo de letras en una sola palabra (bloquea asdfghjkl...)
...
def validate_chat_query(query: str) -> str:
    ...
    for pattern in PROMPT_INJECTION_PATTERNS:
        if re.search(pattern, lowered):
            print(f"🚨 [Guardrails Alert] Intento de Prompt Injection detectado: '{cleaned[:50]}...'")
            raise HTTPException(
                status_code=400,
